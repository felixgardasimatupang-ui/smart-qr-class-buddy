...
-- Students: admins full access, students see own, parents see their children
...
-- Classes: admins full access, authenticated can read
...
-- Class students: admins full access, authenticated can read
CREATE POLICY "Admins can manage class_students" ON public.class_students FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Authenticated can view class_students" ON public.class_students FOR SELECT TO authenticated USING (true);

-- Sessions: admins full access, authenticated can read
...
-- Attendance: admins full access, students can view own, parents can view children's
...
CREATE POLICY "Students can insert own attendance" ON public.attendance FOR INSERT TO authenticated WITH CHECK (
  student_id IN (SELECT id FROM public.students WHERE user_id = auth.uid())
);
CREATE POLICY "Parents can view children attendance" ON public.attendance FOR SELECT TO authenticated USING (
  student_id IN (SELECT id FROM public.students WHERE parent_user_id = auth.uid())
);

-- Evaluations: admins full access, students can view own, parents can view children's
CREATE POLICY "Admins can manage evaluations" ON public.evaluations FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Students can view own evaluations" ON public.evaluations FOR SELECT TO authenticated USING (
  student_id IN (SELECT id FROM public.students WHERE user_id = auth.uid())
);
CREATE POLICY "Parents can view children evaluations" ON public.evaluations FOR SELECT TO authenticated USING (
  student_id IN (SELECT id FROM public.students WHERE parent_user_id = auth.uid())
);
