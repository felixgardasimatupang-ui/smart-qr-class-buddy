...
import { QrCode, CheckCircle, Smartphone, Camera, Sparkles, Wifi, WifiOff, MapPin } from "lucide-react";
import { Html5Qrcode } from "html5-qrcode";
import { Button } from "@/components/ui/button";
import { addPendingAttendance, getPendingCount } from "@/hooks/use-offline-queue";
import { useGeolocation } from "@/hooks/use-sync-manager";
...
export default function StudentScan() {
  ...
  const { geoLocation, getCurrentPosition } = useGeolocation();
  ...
  const handleScan = async (qrCode: string) => {
    ...
    if (!session) {
      toast({ title: "Error", description: "Sesi tidak ditemukan atau tidak aktif", variant: "destructive" });
      return;
    }
    ...
    if (session.qr_expires_at) {
      ...
      const now = new Date();
      if (now > expiresAt) {
        toast({ title: "QR Kedaluwarsa", description: "QR Code sudah tidak berlaku. Minta QR baru dari guru.", variant: "destructive" });
        return;
      }
    }
    ...
    if (isOffline) {
      ...
      setResult("Absensi Disimpan Offline! ✅");
      toast({
        title: "Tersimpan!",
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-md mx-auto space-y-6 animate-in fade-in zoom-in-95 duration-500">
      ...
      {!isOffline && pendingCount > 0 && (
        <div className="flex items-center justify-center gap-2 text-sm text-amber-600 bg-amber-50 dark:bg-amber-950/30 px-4 py-2 rounded-full">
      ...
      {isOffline && (
        <div className="flex items-center justify-center gap-2 text-sm text-destructive bg-destructive/10 px-4 py-2 rounded-full">
          <WifiOff className="w-4 h-4" />
          <span>Mode Offline - Data akan disinkronkan saat online</span>
        </div>
      )}
      ...
      {result ? (
        <Card className="glass-card overflow-hidden border-0 shadow-2xl relative text-center">
          ...
          <CardContent className="py-12 relative z-10 flex flex-col items-center">
            ...
            <p className="text-muted-foreground text-sm font-medium mt-2">
              {isOffline ? "Data absensi disimpan di perangkat." : "Data absensi telah masuk ke sistem pusat."}
            </p>
            ...
            <Button
              ...
              className="mt-8 rounded-full px-8 shadow-lg"
              variant="outline"
            >
        ...
        <Card className="glass-card border-0 shadow-2xl relative overflow-hidden">
          ...
          <CardContent className="p-0">
              ...
              {!scanning && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm z-10 p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground font-medium mb-6">
                    {geoLoading ? "Mendapatkan lokasi..." : "Pastikan pencahayaan cukup dan izinkan akses kamera"}
                  </p>
                  ...
                  >
                    ...
                    {geoLoading ? "Mendapatkan Lokasi..." : "Buka Kamera"}
                  </Button>
                </div>
              )}

              {scanning && (
                <div className="absolute inset-0 border-4 border-primary/50 m-8 rounded-3xl z-10 pointer-events-none"></div>
              )}
              {scanning && (
                <div className="absolute top-1/2 left-8 right-8 h-1 bg-primary z-10 shadow-[0_0_10px_rgba(100,100,255,0.8)] animate-[float_2s_ease-in-out_infinite] pointer-events-none"></div>
              )}
            </div>
            ...
            {geoLocation && (
              <div className="p-3 flex items-center justify-center gap-2 text-xs text-muted-foreground bg-muted/30">
                <MapPin className="w-3 h-3" />
                <span>
                  Lokasi: {geoLocation.lat.toFixed(5)}, {geoLocation.lng.toFixed(5)}
                </span>
              </div>
            )}
          </CardContent>
